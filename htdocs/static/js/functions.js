jQuery(document).ready(function() {
    ...
    $('#form_id').submit(function(e, params) {
        ...
        swal({
            title: "Confirmar",
            text: "Recuerde llenar todos los campos y la información tiene que ser correcta!",
            type: "warning",
    ...
    $('#form_cerrar').submit(function(e, params) {
        var localParams = params || {};
        if (!localParams.send) {
            e.preventDefault();
        }
        ...
        swal({
            title: "Cerrar sesión",
            text: "Se cerrará tu sesión. Tendrás que volver a introducir tu nombre de cuenta y contraseña.",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#6A9944",
    ...
    jQuery(".msg-warning").click(function() {
        swal({
            ...
            swal("¡Hecho!", "Acabas de vender tu alma al diablo.", "success");
        });
    });
    jQuery(".msg-cond").click(function() {
        swal({
            title: "¿Es un nuevo usuario?",
            text: "",
            type: "warning",
            showCancelButton: true,
            confirmButtonColor: "#DD6B55",
            ...
            if (isConfirm) {
                location.href = "/Gicad/contactanos/diagnostico.php";
            } else {
                location.href = "/Gicad/contactanos/solodiagnostico.php";
            }
        });
    });
    ...
    function Input(field) {
        var input = {
            ...
            toHtml: function() {
                ...
                if (input.type !== 'select') {
                    inputTag = t("<input id='{id}' class='{clazz} swal-form-field' type='{type}' name='{name}'" + " value='{value}' title='{placeholder}' placeholder='{placeholder}'" + ' data-swal-forms-required={required}>', input)
                } else {
                    inputTag = t("<select id='{id}' class='{clazz} swal-form-field' name='{name}'" + " value='{value}' title='{placeholder}' style='width:100%'>" + ' data-swal-forms-required={}', input) + input.options.reduce(toHtmlOptions, '') + '</select>'
                }
        ...
        function camelCaseToHuman(arg) {
            if (arg) {
                return arg.replace(/([A-Z])/g, ' $1') // insert a space before all caps
